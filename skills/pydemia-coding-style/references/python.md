...
**예외·한계:** 기존 프로젝트의 서식 기준과 충돌하면 현재 요청과 프로젝트 지침의 적용 범위를 확인한다.
이 규칙만으로 프로젝트의 도구를 변경하거나 파일 전체를 재서식하지 않는다.
...
**이유:** 최근 프로젝트의 import 정렬 설정과 명시적 의존성 지침에 부합한다.
오래된 파일의 import 순서를 개인 표준으로 고정하지 않는다.
